...
`mcp/pyproject.toml` holds a copy for the same reason: Python packaging tools
read the version statically. `VERSION` lists every file holding a copy, as a
reminder of what a bump touches. That list is generated by `--set` and checked by the hook,
...
| `gripper_fault` | — | gFLT as the manual numbers it: `0` none, `5` action delayed, `7` activation required, `8` over temperature, `9` no communication, `10` under voltage, `11` automatic release running, `12` internal fault, `13` activation fault, `14` overcurrent, `15` automatic release complete. The register is four bits wide, so `1`, `2`, `3`, `4` and `6` can also arrive; they are undefined faults, not "no fault" |
| `gripper_fault_severity` | — | How bad the gripper fault is: `0` none, `1` warning, `2` minor, `3` major. A major fault clears only on a reactivation. An undefined code reports `3`, so a switch over the codes above cannot silently drop one |
...
The byte also carries kFLT, the controller's own fault, which nothing exposes. Nobody has needed it, and it is a different failure from the gripper's; open an issue if you do. Both names say `gripper_` for that reason, so adding the controller's pair later needs no rename.

The codes are numbers on the wire, so the driver also names the fault in its own throttled warning, for example `reports gripper fault OverTemperature (code 8)`. An undefined code reads `Unrecognized`. A consumer that wants the name in its own output takes it from the SDK's `toString`.
