...
def grasp_until_contact(
    ...
    spec = tactile.spec_of(gripper_name)
    run = ContactRun(
    ...
    stroke = grippers.stroke_of(gripper_name)
    steps = 0
    state = grippers.get_state(gripper_name)
    reading = tactile.read(gripper_name)
    outcome = stop_before_step(reading, stroke.fingers_met(state.opening_mm), run)
    while outcome is None:
        target_mm = state.opening_mm - run.step_mm
        motion = grippers.move_to_opening(gripper_name, target_mm, max_effort_n)
        steps += 1
        state = grippers.get_state(gripper_name)
        reading = tactile.read(gripper_name)
        outcome = stop_after_step(motion, reading, run) or stop_before_step(
            reading, stroke.fingers_met(state.opening_mm), run
        )

    return ContactGraspResult(
        gripper_name=gripper_name,
        outcome=outcome,
        object_detected=outcome == "contact_detected",
