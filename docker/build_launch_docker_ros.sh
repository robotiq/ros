...
# Configuration
IMAGE_NAME="robotiq_tsf_ros"
DOCKERFILE_PATH="${SCRIPT_DIR}/Dockerfile_TSF85_ROS"
CONTAINER_NAME="robotiq_tsf_ros_container"

echo "=== TSF-85 ROS Docker Build & Launch Script ==="
...
echo "[4/6] Ensuring Docker image exists..."
...
echo "[5/6] Detecting sensor devices..."
DEVICE_ARGS=()
...
echo "passing device ${DEVICE_ARGS[@]}"
...
docker run -it --rm \
  ...
  -v "${PROJECT_ROOT}/robotiq_tsf:/root/noetic_ws/src/robotiq_tsf:rw" \
  "${DEVICE_ARGS[@]}" \
  "${IMAGE_NAME}"
